...
int FM_algo(PLC &plcnet,int gridsize);
...
int main(int argc, const char * argv[])
{
    ...
    for(i=0;i<10;i++)
    {
        min_cost = FM_algo(plcnet,gridsize);
...
bool read_file(char *filename,PLC &plcnet)
{
    ...
    plcnet.parta = new int [plcnet.cell_num/2+1]; //The first step is to move a cell from B to A, so A always needs an extra position
...
int FM_algo(PLC &plcnet,int gridsize)
{
    ...
    {
        //cout<<"max1_inx="<<maxa_inx<<" maxb_inx="<<maxb_inx<<endl;
        plcnet.move_cell(plcnet.partb[plcnet.maxb_inx]); //move to A
        plcnet.cal_all_gains();
        plcnet.in_bin_sort(plcnet.parta,plcnet.maxa_inx);
        
        plcnet.move_cell(plcnet.parta[plcnet.maxa_inx]);
        plcnet.cal_all_gains();
    ...
    if(plcnet.parta_num!=plcnet.partb_num)
    {
        plcnet.move_cell(plcnet.partb[plcnet.maxb_inx]); //move to A
        plcnet.in_bin_sort(plcnet.parta,plcnet.maxa_inx);
        temp_cost = plcnet.cal_total_cost();
    ...
    for(i=0;i<roll_back+1;i++)
    {
        plcnet.move_cell(plcnet.partb[plcnet.maxb_inx]); //move from B to A
        plcnet.cal_all_gains();
        plcnet.in_bin_sort(plcnet.parta,plcnet.maxa_inx);
        
        plcnet.move_cell(plcnet.parta[plcnet.maxa_inx]); //move from A to B
        plcnet.cal_all_gains();
        plcnet.in_bin_sort(plcnet.partb,plcnet.maxb_inx);
    ...
    for(i=plcnet.parta_num-roll_back-1;i<plcnet.parta_num;i++)
    {
        plcnet.parta[i] = plcnet.parta[i+1];
    }
    plcnet.parta[plcnet.parta_num]=-1;
    
    plcnet.maxa_inx = plcnet.parta_num-1;
    plcnet.maxb_inx = plcnet.partb_num-1;
    for(i=0;i<plcnet.cell_num;i++)
    {
        plcnet.cells[i].locked = false;
    }
    plcnet.init_partnum();
    plcnet.init_crossing();
    plcnet.in_bin_sort(plcnet.parta,plcnet.parta_num);
