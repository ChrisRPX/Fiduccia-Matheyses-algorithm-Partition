...
int PLC::cal_node_gain(int cell_No)
{
    ...
    for(i=0;i<cells[cell_No].net_num;i++)
    {
        net_Noo = cells[cell_No].net_No[i];
        if(cells[cell_No].partA_B==0) //from A
        {
            if(nets[net_Noo].parta_num==1) cells[cell_No].gain += 1;
            if(nets[net_Noo].partb_num==0) cells[cell_No].gain -= 1;
        }
        if(cells[cell_No].partA_B==1)
        {
            if(nets[net_Noo].partb_num==1) cells[cell_No].gain += 1;
            if(nets[net_Noo].parta_num==0) cells[cell_No].gain -= 1;
        }
    }
