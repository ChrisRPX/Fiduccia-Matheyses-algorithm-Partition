...
void PLC::re_init()
{
    ...
    init_crossing();
    cal_all_gains();
    find_max(parta,parta_num);
    find_max(partb,partb_num);
...
int PLC::cal_node_gain(int cell_No)
{
    ...
    half_size = cell_num/2;
    for(i=0;i<cells[cell_No].net_num;i++)
    {
        inv_net = cells[cell_No].net_No[i];
        
        if(cells[cell_No].partA_B==0 && nets[inv_net].parta_num>0) //from A
        {
            cells[cell_No].gain += (double)(cc/nets[inv_net].parta_num);
        }
        if(cells[cell_No].partA_B==1 && nets[inv_net].partb_num>0)
        {

            cells[cell_No].gain += (double)(cc/nets[inv_net].partb_num);
        }
        if(nets[inv_net].src_No==cell_No)
        {
            if(cells[cell_No].partA_B==0)
            {
                cells[cell_No].gain += (nets[inv_net].partb_num-nets[inv_net].parta_num+1)*nets[inv_net].cutsize;
            }
            else
            {
                cells[cell_No].gain += (nets[inv_net].parta_num-nets[inv_net].partb_num+1)*nets[inv_net].cutsize;
            }
        }
        else
        {
            for(j=0;j<nets[inv_net].cell_num-1;j++)
            {
                if(nets[inv_net].edge[j].sink_No==cell_No)
                {
                    if(nets[inv_net].edge[j].crossing)
                    {
                        cells[cell_No].gain += nets[inv_net].cutsize;
                    }
                    else cells[cell_No].gain -= nets[inv_net].cutsize;
                }
            }
        }
    }
    ...
    {
        inv_net = cells[cell_No].net_No[i];
        half_size = nets[inv_net].cell_num/2;
        if(cells[cell_No].partA_B==0) //from A
        {
            if(nets[inv_net].parta_num<=half_size && nets[inv_net].parta_num>0)
                cells[cell_No].gain += cc/nets[inv_net].parta_num;
            if(nets[inv_net].parta_num>half_size && nets[inv_net].partb_num>0)
                cells[cell_No].gain -= cc/nets[inv_net].partb_num;
        }
        if(cells[cell_No].partA_B==1)
        {
            if(nets[inv_net].partb_num<=half_size && nets[inv_net].partb_num>0)
                cells[cell_No].gain += cc/nets[inv_net].partb_num;
            if(nets[inv_net].partb_num>half_size && nets[inv_net].parta_num>0)
                cells[cell_No].gain -= cc/nets[inv_net].parta_num;
        }
    }
    */
...
void PLC::find_max(int *part, int part_num)
{
    int i;
    int max_cell=0;
    //if(part_num>0){}
    int temp = cells[part[0]].gain;
    for(i=1;i<part_num;i++)
    {
        if(cells[part[i]].gain>temp)
        {
            temp = cells[part[i]].gain;
            max_cell = i;
        }
    }
    temp = part[part_num-1];
    part[part_num-1] =part[max_cell];
    part[max_cell] = temp;
}
